...
class PoseRAC(pl.LightningModule):
    ...
    def __init__(self, train_x, train_y, valid_x, valid_y, dim, heads, enc_layer, learning_rate, seed, num_classes, alpha):
        ...
        self.transformer_encoder1 = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=dim, nhead=heads),
                                                         num_layers=enc_layer)
        # self.transformer_encoder2 = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=dim, nhead=heads),
        #                                                  num_layers=enc_layer)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # x = self.transformer_encoder2(x)
        # embedding2 = x.view(-1, self.dim)

        # embedding = (embedding1 + embedding2)/2.0
    ...
    def train_dataloader(self):
        ...
        train_loader = DataLoader(dataset, batch_size=128, num_workers=16,shuffle=True)
        return train_loader

    def val_dataloader(self):
        val_dataset = TensorDataset(torch.FloatTensor(self.valid_x), torch.LongTensor(self.valid_y))

        val_loader = DataLoader(val_dataset, batch_size=128, num_workers=16
        ,shuffle=False)
        return val_loader
...
class Action_trigger(object):
    ...
    def __call__(self, pose_score):
        ...
        # if pose_score < self._exit_threshold and self.count>= 5:
        #     self._pose_entered = False
        #     self.count = 0
        #     triggered = True
        if not self._pose_entered:
