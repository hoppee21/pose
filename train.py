...
def normalize_landmarks(all_landmarks):
    ...
    all_landmarks = all_landmarks.reshape(len(all_landmarks), 51)
    return all_landmarks


# For each pose, we use 33 key points to represent it, and each key point has 3 dimensions.
# Here we obtain the pose information (17*2=34) of each key frame, and set up the label (1 for salient pose I and 0 for salient pose II).
...
def obtain_landmark_label_old(csv_path, all_landmarks, all_labels, label2index, num_classes):
    file_separator=','
    n_landmarks = 17
    n_dimensions = 2
    ...
    with open(csv_path) as csv_file:
        ...
        for row in csv_reader:
            ...
            all_landmarks.append(landmarks)
            label = label2index[row[1]]
            period = row[0].split('/')[-3]
            index = int(row[0].split('/')[-1].split('.')[0])

            
            label_np = np.zeros(num_classes)
            label_np[label] = curve_fitting(index, period)
            all_labels.append(label_np)
    return all_landmarks, all_labels       

def curve_fitting(index, period):
    up = [10, 10, 3, -3, -10, -10]
    down = [-10, -10, 3, -3, 10, 10]
    if period == "period1":
        return up[index]
    else:
        return down[index]
